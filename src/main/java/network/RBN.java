...
public class RBN<K, V> extends AbstractBooleanNetwork<K, V> {

    protected final Random random;
    protected final int k;
    protected Supplier<Table<K, V>> tableSupplier;

    private RBN(int nodesNumber, int k, Random random, Supplier<Table<K, V>> tableSupplier) {
        super(nodesNumber);
        this.k = k;
        this.random = random;
    ...
    public static <K,V> BooleanNetwork<K,V> newInstance(int nodesNumber, int k, Random random, Supplier<Table<K, V>> tableSupplier){
        return new RBN<K,V>(nodesNumber, k, random, tableSupplier);
    }
